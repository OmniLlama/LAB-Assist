...
export class InputEditorEvents {
  ...
  static Part_lMouDown(e): void {
    ...
    if (e.ctrlKey) {
      ...
      iec.currPart = null;
      if (iec.currNote !== null) { InputEditorFunctions.unselectNote(iec.currNote); }
      iec.currNote = null;
    } else {
   ...
   * @param e
   */
  static Note_MouOver(e): void { (e.target as HTMLDivElement).style.cursor = 'move'; }
  /**
 ...
 */
   ...
   * @param e
   */
  static NoteEdge_Left_MouOver(e: MouseEvent): void { (e.target as HTMLDivElement).style.cursor = 'w-resize'; }
  /** Event: mouse over right note edge
   * @param e
   */
  static NoteEdge_Right_MouOver(e: MouseEvent): void { (e.target as HTMLDivElement).style.cursor = 'e-resize'; }
  ...
  static NoteEdge_Left_lMouDown(e: MouseEvent): void {
    ...
    if (tmp_note == undefined) { tmp_note = iec.changingNote; }
    if (iec.changingNote == null) { iec.changingNote = tmp_note; }
    if (iec.heldEdge == null) { iec.heldEdge = e.target; }
  ...
  static Generic_lMouDown(e: MouseEvent) {
    ...
    if (tmp_className.indexOf('note') !== -1) {
      if (iec.currNote !== null) { InputEditorFunctions.unselectNote(iec.currNote); }
      iec.currNote = iec.allNotes[elmt.id];
      if (iec.currNote !== null) { InputEditorFunctions.selectNote(iec.currNote); }
      iec.currPart = iec.currNote.part;
      if (iec.currPart !== null) { InputEditorFunctions.selectPart(iec.currPart); }
      return;
    } else if (tmp_className.indexOf('part') !== -1) {
      // keyEditor.setPlayheadToX(e.pageX);
      if (iec.currPart !== null) { InputEditorFunctions.unselectPart(iec.currPart); }
      iec.currPart = iec.allParts[elmt.id];
      if (iec.currPart !== null) { InputEditorFunctions.selectPart(iec.currPart); }
      if (iec.currNote !== null) { InputEditorFunctions.unselectNote(iec.currNote); }
      iec.currNote = null;
  ...
  static initInputEvents() {
    ...
    iec.keyEditor.addEventListener('scale draw', () => { InputEditorVisuals.draw(iec); });
     ...
     */
    iec.html.div_Score.addEventListener('dblclick', (me) => { InputEditorEvents.Grid_lMouDbl(me); });
    ...
    iec.html.div_Score.addEventListener('mousedown', (me) => { InputEditorEvents.Generic_lMouDown(me); });
    ...
    iec.html.btn_AddPart.addEventListener('click', () => { InputEditorFunctions.addRandomPartAtPlayhead(iec); });
    /**
     * Sliders
     */
 ...
 */
  static initWindowEvents(iec: InputEditorComponent) {
    ...
    window.addEventListener('resize', (e) => { InputEditorVisuals.resize(); }, false);
  }
  /**
